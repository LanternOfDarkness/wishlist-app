...
_Date: 2026-08-01 · Status: implemented (see [`plan-sketch-redesign.md`](./plan-sketch-redesign.md)) · Scope: sitewide — see the note below, superseded from the original marketing-only scope_
...
**Scope note (superseded):** this doc originally scoped the sketch identity to the *marketing surface* only, keeping `src/lib/wishlist-appearance.ts` and the per-user color presets untouched. The implementation plan reopened that: the sketch **structure** (double-stroke boxes, hatch fills, wobble icons, handwritten headings) is brand-fixed and applies sitewide, including the wishlist page and embed — but **color** on those two surfaces still comes from each user's own `resolveWishlistAppearance` preset, never a hardcoded brand hex. The brand palette itself was then added as two new opt-in user presets, `paper` and `chalk`, seeded as the default for new wishlists only. See [`plan-sketch-redesign.md`](./plan-sketch-redesign.md) §0 for the full two-axis rule.
...
| `--text-muted` | `#6E6A5F` (corrected from this doc's original `#7A756A`, which fails WCAG AA at 4.29:1 — `#6E6A5F` gives 5.06:1) | `#A8ACA6` | secondary copy |
...
Dark mode must be implemented as separate token values (not a CSS `invert()` or opacity trick). **Implementation note:** the site's actual dark mode is the `.dark` class toggled by `next-themes` (`attribute="class"`), not a media query — the light values live in `:root`, the dark values under `.dark`, and the mockup's `@media (prefers-color-scheme: dark)` + `[data-theme="dark"]` pattern was intentionally *not* used, since an explicit light choice would otherwise lose to the OS setting.
...
- **Implementation note — Cyrillic gap:** neither `Architects Daughter` nor `Patrick Hand` covers Cyrillic, and the app ships a `uk` locale. Each primary face is paired with a Cyrillic-capable companion via CSS font-fallback (`"Architects Daughter", "Caveat", cursive` for display; `"Patrick Hand", "Pangolin", cursive` for body) so Ukrainian copy renders in the companion face automatically, per-glyph, with no locale branching. All four families are self-hosted under `public/fonts/`.
...
**Ruled paper background** — a very faint (`opacity: 0.12`) horizontal repeating-linear-gradient across the whole page, evoking loose-leaf paper. **Implementation note:** attached via an opt-in `.paper-rules` wrapper class (used on the landing page only), not a `body::before` fixed layer — this lets the embed iframe and other short/plain surfaces skip the texture.
...
5. **Personalize showcase** — mini preset cards generated by mapping over the real `COLOR_PRESET_OPTIONS` from `wishlist-appearance.ts`, so this section stays truthful to what the product actually offers and can't drift out of sync. At the time this doc was written that was 5 presets (`light`, `rose`, `green`, `dark`, `minimal`); the implementation later added two brand-derived presets (`paper`, `chalk`, see the plan's Phase 4), which the showcase picks up automatically.
...
## 6. Open items — resolved during implementation

- **Hand-drawn motif reach:** went sitewide, not landing-only — structure (double-stroke boxes, hatch fills, wobble icons, handwritten headings) applies everywhere including the wishlist page and embed, while dashboard/settings forms keep plain UI type for form-control legibility (`Input`/`Label`/`Select` were explicitly left untouched — see plan Phase 5). This is the two-axis rule in §0 of the plan.
- Fonts are self-hosted via `next/font/local` from `public/fonts/`, no data-URI, no `fonts.googleapis.com` at runtime.
- All new copy is wired through `next-intl` with real Ukrainian translations in `messages/uk.json`, not English placeholders.
- No new brand name was chosen — the existing wordmark text stays; only the visual identity changed.
